...
class GetBetsRouter(TGRouter):
    ...
    def setup(self, handler: MakeBetsHandler) -> None:
        self.message.register(self.handler(handler.ask_who_make_bet), GameState.START_ROUND, F.text.regexp(r'Start the \w+ round'))
        self.message.register(self.handler(handler.handle_username), RoundState.WAIT_USERNAME_TO_BET)
        self.message.register(self.handler(handler.handle_bet), RoundState.WAIT_BET_OF_PLAYER, F.text.regexp(r'\d+'))
...
def create_make_bets_router(bot: TGBot,
    ...
    handler = MakeBetsHandler(bot, player_data_service, game_data_service)
