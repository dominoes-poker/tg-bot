...
def setup_router(router: TGRouter, handler: ExternalPlayerRegisterHandler) -> None:
...
def create_external_player_register_router(bot: TGBot,
                                           player_data_service: PlayerDataService) -> TGRouter:
    router = TGRouter(name='<ExternalPlayerRegister> - Router')
